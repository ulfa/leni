...
content_types_provided(ReqData, Context) ->
    {[{"image/png", provide_content}, {"text/html", provide_logging}],ReqData, Context}.
%
...
provide_content(ReqData, Context) ->
    [Node,Name] = wrq:path_tokens(ReqData) ,
    Content = rpc:call(list_to_atom(Node), horst, get_pic, [Name]),
    lager:debug("get the file : ~p from node : ~p", [Name, Node]),
    {Content, ReqData, Context}. 

provide_logging(ReqData, Context) ->
    [Node,Name] = wrq:path_tokens(ReqData) ,
    Content = rpc:call(list_to_atom(Node), horst, get_log, [Name]),    
    lager:debug("get the file : ~p from node : ~p", [Name, Node]),
    {binary_to_list(Content), ReqData, Context}. 
